...
/*
 * Modified version, by Scott Benton.
...
spl_autoload_register( function( $classname ) {
	...
	// Let the base CI files still be loaded as normal.
	if ( strpos( $classname, 'CI_' ) !== FALSE )
	{
		$classfile = substr( $classname, strpos( $classname, 'CI_' ) + 3 );
		$classfile = BASEPATH . 'core'. DIRECTORY_SEPARATOR . $classfile . '.php';
		require_once( $classfile );
	}
	// Let the custom core files be loaded too.
	elseif ( strpos( $classname, 'MY_' ) !== FALSE )
	{
		$classfile = APPPATH . 'core'. DIRECTORY_SEPARATOR . $classname . '.php';
		require_once( $classfile );
	}
	// Search for namespaced classes?
